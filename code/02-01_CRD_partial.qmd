...
```{r ANOVA}
anova(crd_mod) #same result as type 3 if no data missing so it is a safe option to use Anova() which uses type 3 sum of squares.
...
ggplot(crd_resid, aes(x = .fitted,
                      y = .studresid))+geom_point(shape = 21, fill = "purple",
                                                  size = 3,
                                                  alpha = .7)+
...
crd_cld_selected <- crd_cld_all %>%
  ...
  mutate(trtname= paste(nrate_kgha, "+", krate_kgha))
crd_cld_selected
```
...
ggplot()+
  ...
  geom_boxplot(data = crd_dfw, aes(x = krate_kgha, y = yield_kgha))+
  # Adding letters
